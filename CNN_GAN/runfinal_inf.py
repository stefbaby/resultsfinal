...
def generator(z, alpha=0.2):
    ...
    with tf.variable_scope("generator"):        
        ...
        conv1 = tf.layers.Conv1D(filters = 64,kernel_size = 1,strides = 1, padding='SAME')(z)
        conv1 = tf.layers.batch_normalization(conv1)#, training=training)
        conv1 = lrelu(conv1, alpha)

        conv2 = tf.layers.Conv1D(filters = 128,  padding='SAME', strides = 1, kernel_size = 1)(conv1)
        ...
        out =  tf.nn.softmax(gumbel_softmax_sample /2.0) # by temp
...
def discriminator(x,reuse=False, alpha=0.2, training=True):
    ...
    with tf.variable_scope("discriminator"):
        conv1 = tf.layers.Conv1D(filters = 64,kernel_size = 1,strides = 1, padding='SAME')(x)
        ...
        conv3= tf.layers.Conv1D(filters=128,padding='SAME',strides = 1, kernel_size = 1)(conv2)
        conv3 = tf.layers.batch_normalization(conv3)#, training=training)
        conv3 = lrelu(conv3, alpha)

        # Flatten it
        flat = tf.reshape(conv3, (-1, 19*128))
...
def run_inference(A, B, B_fake_probs, dictionary, num_examples):
    print("running inference")
    B_fake_probs = np.argmax(B_fake_probs, axis = -1)
    print("shape of B and B'", B.shape , B_fake_probs.shape)
    nottoinclude = ['<PAD>', '<START>' , '<END>', '<UNK>']
    for i in range(num_examples):
        gensen = ids_to_text(A[i], dictionary).split() + ['because'] + ids_to_text(B[i], dictionary).split()
        gensen = ' '.join([word for word in gensen if word not in nottoinclude])
        print("A: Bgen")
        print(gensen)
        csen = ids_to_text(A[i], dictionary).split() + ['because'] + ids_to_text(B_fake_probs[i], dictionary).split()
        csen = ' '.join([word for word in csen if word not in nottoinclude])
        print(csen)
        csen = re.sub("[^a-zA-Z]"," ", csen)
        gensen = re.sub("[^a-zA-Z]"," ", gensen)
        wh = open('forbimpm_double.txt', 'a')
        if(random.random() < 0.5): #Randomly choosing the order
            wh.write('\t'.join(['first',csen,gensen]) + '\n')
        else:
            wh.write('\t'.join(['second',gensen,csen]) + '\n')
        # print("B:")
...
def run_a_gan(sess, G_train_step, G_loss, D_train_step, D_loss, G_extra_step, D_extra_step,\
              G_sample, saver, save_every = 100, print_every=14, batch_size=128,shuffle = False,\
              num_epochs = 90000000):
    ...
    A = np.load('../../cs221data/A_v500_blen19.npy')
    A = A.astype(int)
    A = A[:num_train_examples + num_dev_examples,:14]



    B = np.load('../../cs221data/B_v500_blen19.npy', encoding = 'latin1')
    B = B.astype(int)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % print_every == 0:
            ...
            run_inference(A_dev_id, B_dev_id, generated_probs[0], dictionary, num_dev_examples)
            break
    #if epoch % save_every == 1:
    #    save_path = saver.save(sess, "../../results/")
...
# get our solver
D_solver, G_solver = w_get_solvers()

# get our loss
D_loss, G_loss = w_gan_loss(logits_real, logits_fake)
