...
experiment = Experiment(api_key="",project_name="cs221", workspace="cs221gan")
...
def dev_eval(A_dev, B_dev, batch_size, sess):
    lengths = np.ones((batch_size), dtype=int) * max_seq_length
    epoch_G_loss = 0
    epoch_D_loss = 0
    ...
    for i in (range(batches)):
        ...
        G_loss_curr, D_loss_curr = sess.run([G_loss, D_loss], feed_dict={b: minibatch_b,a: minibatch_a})
...
def run_a_gan(sess, G_train_step, G_loss, D_train_step, D_loss, G_extra_step, D_extra_step,\
    ...
    A = A.astype(int)
    A = A[~(labels == 0)]
    A = A[num_train_examples + batch_size]


    B = np.load('../../data/B_19.npy', encoding = 'latin1')
    B = B.astype(int)
    B = B[~(labels== 0)]
    B = B[num_train_examples:num_train_examples + batch_size]
    ...
    A_dev = A[num_train_examples:num_train_examples+batch_size]
    A = A[:num_train_examples]

    B_dev = B[num_train_examples:num_train_examples+batch_size]
